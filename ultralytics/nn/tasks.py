...
DETECT_CLASS = (Detect, Detect_Efficient)
V10_DETECT_CLASS = (v10Detect,)
SEGMENT_CLASS = (Segment,)
POSE_CLASS = (Pose,)
OBB_CLASS = (OBB,)
C3K2_CLASS = (C3k2_ConvFormerCGLU,)
A2C2F_CLASS = ()
C2PSA_CLASS = (C2BRA,)
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):
    """Parse the reduced ISD-YOLO graph without importing unrelated modules."""
    import ast

    nc = d.get("nc", 0)
    scales = d.get("scales") or {}
    scale = d.get("scale") or (next(iter(scales)) if scales else "")
    depth, width, max_channels = scales.get(scale, (1.0, 1.0, float("inf")))[:3]
    if scales and not d.get("scale"):
        LOGGER.warning(f"WARNING no model scale passed. Assuming scale='{scale}'.")

    supported = {
        "Conv": Conv,
        "C3k2": C3k2,
        "C3k2_ConvFormerCGLU": C3k2_ConvFormerCGLU,
        "SPPF": SPPF,
        "C2BRA": C2BRA,
        "DAttention": DAttention,
        "Concat": Concat,
        "Detect_Efficient": Detect_Efficient,
    ...
    for i, (f, n, module_name, args) in enumerate(d["backbone"] + d["head"]):
        m = supported.get(module_name)
        if m is None:
            raise KeyError(f"ISD-YOLO parser does not support module {module_name!r}")
        args = list(args)
        ...
        c1 = channels[f] if isinstance(f, int) else sum(channels[x] for x in f)

        if m in (Conv, C3k2, C3k2_ConvFormerCGLU, SPPF, C2BRA):
            c2 = args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8) if c2 != nc else c2
            if m in (C3k2, C3k2_ConvFormerCGLU, C2BRA):
                args = [c1, c2, n_, *args[1:]]
                n_inst = 1
            else:
                args = [c1, c2, *args[1:]]
                n_inst = 1
            ...
            n_inst = n_
        elif m is DAttention:
            c2 = c1
            args = [c1, *args]
            n_inst = n_
        elif m is Detect_Efficient:
            c2 = sum(channels[x] for x in f) if isinstance(f, list) else channels[f]
            args = [*args, [channels[x] for x in f]]
            n_inst = n_
        else:
            raise AssertionError(f"Unhandled ISD-YOLO module {m}")

        module = nn.Sequential(*(m(*args) for _ in range(n_inst))) if n_inst > 1 else m(*args)
        ...
        if verbose:
            LOGGER.info(f"{i:>3}{str(f):>20}{n_:>3}{module.np:10.0f}  {module.type:<35}{str(args):<30}")
